...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("🌏 ArkaLife RolePlay", "https://cdn.discordapp.com/attachments/819636936049688626/833428749940424754/96f791082c690416a0507caab1de6531.png")
    .setColor(0x722ea5)
    .setFooter(TICK_N % 2 === 0 ? '⚪ <-- REFRESH | ArkaLife RolePlay' : '⚫ <-- REFRESH | ArkaLife RolePlay')
    .setTimestamp(new Date())
    .addField('\n\u200b\nComment pouvez-vous nous rejoindre sur 🌏 ArkaLife RolePlay :','*Lancer FiveM ➔ F8 ➔* `connect Arkalife.isaper.com`\n\u200b\n─────────────────────────────────────────────────',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Le serveur est actuellement :',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    if (LAST_COUNT !== null) log(LOG_LEVELS.INFO,`Serveur ETEINT ${URL_SERVER} (${URL_PLAYERS} ${URL_INFO})`);
    let embed = UpdateEmbed()
    .setColor(0x722ea5)
    .addField('Status du serveur',':x: Hors Ligne',true)
    .addField('File d\'attente','*Aucune en cours...*',true)
    .addField('Joueurs connectés','*0*\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Status du serveur',':white_check_mark: En Ligne !',true)
  ...
  bot.on('message', async function (msg) {
    if (msg.channel.id === '815155548327903253') {
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag,message.author.displayAvatarURL)
          .setColor(0x722ea5)
          .setTitle('Report d\'un bug, validé !')
          .setDescription('Le report à bien été envoyé au staff, merci !')
          .setTimestamp(new Date());
